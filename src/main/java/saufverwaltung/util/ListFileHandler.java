...
public class ListFileHandler {
	...
	private final String lineSeparator = times("-", colWidths.get(0)) + "+" + times("-", colWidths.get(1) + 1) + "+"
			+ times("-", colWidths.get(2) + 1) + "+" + times("-", colWidths.get(3));
	...
	public void writeListToFile(List<Member> memberlist) {
		...
		try {
			...
			w.write(padRight(listname, colWidths.stream().reduce(0, (a, b) -> a + b) - dateString.length() + 5)
					+ dateString);
			w.newLine();
			w.newLine();
			...
			for (Member cm : memberlist) {
				...
				String blockedStr = padRight(cm.getGuthaben() < 0 ? blocked : " ", colWidths.get(1));
				w.write(padRight(cm.getName(), colWidths.get(0)) + "| " + blockedStr + "| " + blockedStr + "|"
